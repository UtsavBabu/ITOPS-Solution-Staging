...
export function UptimeVisual() {
  ...
  return <div className="mockup-dark rounded-2xl p-5">
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-center">
        {[["126ms", "avg response"], ["30s", "check interval"], ["5", "redirects traced"]].map(([v, l]) => <div key={l} className="rounded-lg border border-white/10 bg-black/30 py-2">
      ...
      <div className="flex items-center gap-5">
        <div className="relative h-24 w-24 shrink-0">
          <svg viewBox="0 0 84 84" className="h-full w-full -rotate-90">
            ...
            <motion.circle cx="42" cy="42" r={R} fill="none" stroke="#34d399" strokeWidth="7" strokeLinecap="round" strokeDasharray={C} initial={{
            strokeDashoffset: C
          }} whileInView={{
        ...
        <ul className="flex-1 space-y-2 text-xs">
          {[["strict-transport-security", true], ["content-security-policy", true], ["x-frame-options", true], ["permissions-policy", false]].map(([h, ok], i) => <motion.li key={String(h)} initial={{
          opacity: 0,
          x: 8
        }} whileInView={{
          opacity: 1,
...
export function HostsVisual() {
  const rows = [["CPU", 31, "bg-cyan-400"], ["Memory", 58, "bg-blue-400"], ["Disk", 44, "bg-violet-400"]];
  return <div className="mockup-dark rounded-2xl p-5">
      <div className="flex items-center justify-between text-xs">
      ...
      <div className="mt-4 space-y-3">
        {rows.map(([label, pct, tone], i) => <div key={label}>
...
export function IncidentVisual() {
  const steps = [["14:02:11", "checkout-service failed 2 consecutive checks", "text-red-300"], ["14:02:12", "Incident opened · Slack + email alert sent", "text-amber-300"], ["14:07:40", "Checks passing again", "text-cyan-300"], ["14:07:41", "Incident auto-resolved · recovery alert sent", "text-emerald-300"]];
  return <div className="mockup-dark rounded-2xl p-5">
      <p className="text-xs text-white/60">Incident #2481 · timeline</p>
      <div className="mt-4 space-y-0">
        {steps.map(([t, msg, tone], i) => <motion.div key={t} initial={{
        opacity: 0,
        y: 8
      }} whileInView={{
        opacity: 1,
...
export function ShowcaseRow({
  ...
  return <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
      <motion.div initial={{
      opacity: 0,
      x: flip ? 24 : -24
    }} whileInView={{
      ...
      ease: EASE
        ...
        <h3 className="mt-3 text-2xl font-semibold tracking-tight text-white light:text-slate-900 md:text-3xl">{item.title}</h3>
        <p className="mt-4 max-w-lg text-sm leading-relaxed text-white/60 light:text-slate-500 md:text-base">{item.body}</p>
        <ul className="mt-6 space-y-2.5">
          {item.bullets.map(b => <li key={b} className="flex items-start gap-2.5 text-sm text-white/70 light:text-slate-600">
        ...
        <Link to={item.href} className="group mt-7 inline-flex items-center gap-1.5 text-sm font-medium text-cyan-300 light:text-cyan-600 transition-colors hover:text-cyan-200">
          {item.cta}
