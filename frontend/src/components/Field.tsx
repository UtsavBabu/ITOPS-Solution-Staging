...
type Validity = "idle" | "error" | "success";
...
export function Field({ label, icon, error, success, hint, showStrength, type = "text", value, ...props }: FieldProps) {
  const id = useId();
  const [reveal, setReveal] = useState(false);
  const isPassword = type === "password";
  const inputType = isPassword && reveal ? "text" : type;
  const validity: Validity = error ? "error" : success ? "success" : "idle";
  const pw = typeof value === "string" ? value : "";
  const strength = showStrength && isPassword ? strengthOf(pw) : null;

  const ring =
    validity === "error"
      ? "border-red-400/50 focus-within:border-red-400/70 focus-within:shadow-[0_0_0_4px_rgba(248,113,113,0.12)]"
      : validity === "success"
        ? "border-emerald-400/50 focus-within:shadow-[0_0_0_4px_rgba(52,211,153,0.12)]"
        : "border-white/12 focus-within:border-cyan-400/50 focus-within:shadow-[0_0_0_4px_rgba(34,211,238,0.14)]";

  return (
    <div>
      <div className={`group relative rounded-xl border bg-black/40 transition-all duration-200 ${ring}`}>
        {icon && (
          <span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-white/40 transition-colors group-focus-within:text-cyan-300">
            {icon}
          </span>
        )}
        <input
          ...
          value={value}
          placeholder=" "
          aria-invalid={validity === "error"}
          className={`peer h-14 w-full rounded-xl bg-transparent pb-2 pt-5 text-sm text-white placeholder-transparent focus:outline-none ${
            icon ? "pl-11" : "pl-4"
          } ${isPassword || validity !== "idle" ? "pr-11" : "pr-4"}`}
          {...props}
        />
        <label
          htmlFor={id}
          className={`pointer-events-none absolute top-2 text-xs font-medium text-cyan-300/90 transition-all duration-200
        ...
        {isPassword ? (
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg p-1.5 text-white/40 transition-colors hover:text-white"
          >
            {reveal ? <EyeOff /> : <Eye />}
          </button>
        ) : validity === "success" ? (
