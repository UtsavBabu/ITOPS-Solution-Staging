-- Real ICMP ping, relayed through the Kada Nigrani agent.
--
-- The cloud edge runtime has never been able to send a raw ICMP packet (no
-- raw sockets) — flagged "on the roadmap" repeatedly rather than faked. The
-- agent, though, is a real process on a real Linux host with the system
-- `ping` binary already available, so — exactly like the TCP relay added in
-- migration 0067 — a PING check is real once it's always delivered via an
-- agent. Unlike TCP, PING has no cloud-direct fallback: there is no such
-- thing as "ping from the cloud" here, so it's agent-only by constraint.
...
alter table monitors add constraint monitors_check_type_check
  check (check_type in ('HTTP', 'KEYWORD', 'STATUS_CODE', 'DNS', 'TCP', 'PING'));
...
alter table monitors add constraint monitors_via_agent_check_type
  check (via_host_agent_id is null or check_type in ('TCP', 'PING'));

alter table monitors drop constraint if exists monitors_ping_requires_agent;
alter table monitors add constraint monitors_ping_requires_agent
  check (check_type <> 'PING' or via_host_agent_id is not null);

drop function if exists create_monitor(text, text, text, text, text, text, int, text, text, int, uuid);
...
begin
  ...
  if p_check_type not in ('HTTP', 'KEYWORD', 'STATUS_CODE', 'DNS', 'TCP', 'PING') then
  ...
  if p_check_type = 'PING' and p_via_host_agent_id is null then
    raise exception 'ICMP ping requires an agent to relay it — pick one under "Check via"';
  end if;

  if p_via_host_agent_id is not null then
    if p_check_type not in ('TCP', 'PING') then
      raise exception 'Only device/TCP or ping checks can be relayed via an agent';
    end if;
    if not exists (
  ...
  v_asset_type := case
    when p_check_type in ('TCP', 'PING') then 'NETWORK'
