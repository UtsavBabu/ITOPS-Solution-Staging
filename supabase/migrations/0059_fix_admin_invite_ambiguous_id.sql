-- Real bug caught by live end-to-end testing immediately after 0058 shipped:
-- "column reference \"id\" is ambiguous". RETURNS TABLE (id uuid, token
-- text) implicitly declares `id` as a plpgsql variable visible through the
-- whole function body — the bare `where id = p_organization_id` in the
-- org-existence check could mean that OUT parameter or organizations.id.
...
begin
  ...
  if not exists (
    ...
    where r.key = p_role and r.scope = 'organization' and (r.organization_id is null or r.organization_id = p_organization_id)
  ) then
